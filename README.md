...
You need Python 3.9+ and (optionally, for PDF output) LibreOffice. PyYAML is the only required Python dependency; the optional calendar feature also needs `caldav` and `icalendar`.

```bash
git clone https://github.com/biterik/travel-forms-pilot.git
cd travel-forms-pilot
pip install pyyaml --break-system-packages       # macOS / system Python
# or: pip install pyyaml --user
```
