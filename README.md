...
- **Dashboard** — a single self-contained HTML file (`dashboard.py`) giving an action-first overview of every trip: status, deadlines, missing applications, registration cut-offs, and pending reimbursements. Portable (pure Python + PyYAML, no internet, no LLM needed to view it), opens in any browser on macOS/Linux/Windows. See [Dashboard](#dashboard-overview-across-trips).
- **Calendar** — on request it drops a trip into your calendar via CalDAV (set up for Kerio Connect), confirm-first.
...
2. Open the assistant in your `TRAVEL-FORMS` directory and point it at the
   ...
   and the **registration / early-bird deadline**.
3. You get a ready-to-sign **Dienstreiseantrag** PDF. Sign it and email it to
   the Reisestelle. (It also offers to add the trip to your calendar.)
...
- `identity.yaml` — your personnel number, cost center, bonus-programme membership, and (for the optional calendar feature) your CalDAV login in a `kalender:` block. For the password you have two options: **either** leave `app_password` blank and let the calendar script prompt you securely at runtime, **or** paste a Kerio **app password** into `app_password` to skip the prompt. (Note: Kerio app passwords don't work for Active-Directory-imported accounts — a known KADE bug — so for those, leave it blank and enter your regular password when prompted.)
...
applications as a trip nears, filed-but-not-reimbursed, and **registration /
early-bird deadlines** (from the `anmeldung:` block in `trip.md`). The dashboard
is read-only: spot something, edit that trip's `trip.md`, and re-run. Only trips
...
- Backlog import (`backlog_trip.py`): lenient importer for old single-trip folders — infers status, recovers trip number / destination / dates / purpose, fills `trip.md`, sorts files. Non-destructive, `--dry-run` supported.
